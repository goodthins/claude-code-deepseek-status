...
| `CLAUDE_EFFORT` | Effort level: `max` / `high` / `medium` / `low` |
...
**Q: The effort shows MAX — is it really using max?**
A: Yes. The plugin reads `CLAUDE_EFFORT` — the same env var Claude Code uses to set the thinking budget. What you see is what's being used.
...
| `CLAUDE_EFFORT` | 努力级别：`max` / `high` / `medium` / `low` |
...
**Q: 努力级别显示 MAX，是真的在用 max 吗？**
A: 是的。插件直接读取 `CLAUDE_EFFORT` 环境变量——这正是 Claude Code 用来决定 thinking budget 的同一个变量。显示的值就是实际使用的值。
